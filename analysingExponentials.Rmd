...
On the first glance, these values look quite good and are close to the expected values. A simple Kolmogorov-Smirnov-Test for normal distribution verifies it:

```{r, warning=F, message=F}
library(ks)
ks.test(randomMeans, "pnorm", mean=5, sd=0.79)$p.value
```

The p-value is quite high and thus the null hypothesis (the mean is normally distributed) is not rejected.
...
It looks quite normal, as the above test, the approx. equal standard deviations and means and the central limit theorem suggests.